...
async function main() {
  ...
  if (count > 0) {
    console.log(`Seed skipped — ${count} supplier(s) already present.`);
    return;
  }
  ...
  await db.insert(suppliers).values(
    SUPPLIERS.map(([legalName, dbaName, companyType, ticker, taxClass, category, city, state, zip], i) => ({
  ...
  console.log("Tip: open the app and click 'Refresh' on a row to pull market news and raise risk alerts.");
}
